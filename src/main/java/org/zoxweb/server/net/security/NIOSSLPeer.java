 ...
 * which is common for both client and server and provides the abstract {@link NIOSSLPeer#read(SocketChannel, SSLEngine, ByteBuffer)} and
 * {@link NIOSSLPeer#write(SocketChannel, SSLEngine, ByteBuffer)} methods, that need to be implemented by the specific SSL/TLS peer
...
public abstract class NIOSSLPeer {
     ...
     * is applied on {@link NIOSSLPeer#}. It should be initialized using {@link SSLSession#getPacketBufferSize()},
    ...
    protected NIOSSLPeer(Executor executor) {
        this.executor = executor;
    }

    protected abstract ByteBuffer read(SocketChannel socketChannel, SSLEngine engine, ByteBuffer buffer) throws Exception;

    protected abstract void write(SocketChannel socketChannel, SSLEngine engine, ByteBuffer buffer) throws Exception;
