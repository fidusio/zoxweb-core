...
public class ReflectionUtil {
    ...
    public static class MethodAnnotations {
        ...
        public boolean isMethodAnnotatedBy(Class<? extends Annotation> a) {
    ...
    public static class AnnotationMap {
        ...
        public boolean isClassAnnotatedByType(Class<? extends Annotation> type) {
            return findClassAnnotationByType(type) != null;
        }

        public boolean isMethodAnnotatedByType(Method m, Class<? extends Annotation> type) {
            return findMethodAnnotationByType(m, type) != null;
        }


        public MethodAnnotations lookupMethodAnnotations(Method method) {
            return methodsAnnotations.get(method);
        }

        public synchronized MethodAnnotations[] findMethodAnnotationsByType(Class<? extends Annotation> type) {
            MethodAnnotations[] all = methodsAnnotations.values().toArray(new MethodAnnotations[0]);
            List<MethodAnnotations> ret = new ArrayList<>();
            for (MethodAnnotations ma : all) {
                if (ma.isMethodAnnotatedBy(type))
                    ret.add(ma);
            }

            return ret.isEmpty() ? null : ret.toArray(new MethodAnnotations[0]);
        }


        public <V extends Annotation> V findMethodAnnotationByType(Method m, Class<? extends Annotation> type) {
            SUS.checkIfNulls("Method or class can't be null", m, type);
            MethodAnnotations methodAnnotations = methodsAnnotations.get(m);

            if (methodAnnotations != null) {
                return (V) methodAnnotations.methodAnnotations.get(type);
            }
            return null;
        }

        public <V extends Annotation> V findClassAnnotationByType(Class<? extends Annotation> type) {
            SUS.checkIfNulls("Class can't be null", type);
